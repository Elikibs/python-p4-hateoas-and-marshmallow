...
class NewsletterByID(Resource):
    ...
    def get(self, id):
        ...
        newsletterbyid = Newsletter.query.filter_by(id=id).first()

        response = make_response(
            newsletter_schema.dump(newsletterbyid),
            200,
        )
    ...
    def patch(self, id):
        ...
        for attr in request.form:
            setattr(newsletterbyid, attr, request.form[attr])

        db.session.add(newsletterbyid)
        db.session.commit()
    ...
    def delete(self, id):
        ...
        db.session.delete(newsletterbyid)
        db.session.commit()
